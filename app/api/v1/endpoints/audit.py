...
Tenant-scoped (multi-tenancy extension): a tenant `admin` only sees their
own tenant's audit trail; a platform superuser sees everything (optionally
...
async def list_audit_logs(
    ...
    base_query = _filtered_query(current_user, tenant_id, user_id, action, entity_type, entity_id)
...
async def export_audit_logs(
    ...
    query = _filtered_query(current_user, tenant_id, user_id, action, entity_type, None).order_by(
    ...
    writer.writerow(
        ["log_id", "tenant_id", "user_id", "action", "entity_type", "entity_id", "ip_address", "created_at"]
    )
    for log in logs:
        writer.writerow(
            [
