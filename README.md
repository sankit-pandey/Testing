...
`../.github/copilot-instructions.md`), extended with **multi-cloud
storage/embeddings** and **Poetry** dependency management per direct
follow-up instruction (beyond the original locked design — see §6 below for
...
```bash
alembic upgrade head          # apply the locked schema (Database_Schema.md, verbatim)
...
├── models/                   # SQLAlchemy models (Database_Schema.md's 14 tables, verbatim)
...
Three capabilities were added by direct follow-up instruction, **beyond**
`Database_Schema.md`/`LOCKED_Design_v1.0.md` (which assumed S3-only storage,
`pip`, and a containerized Redis). Each is additive/config-driven and does
not touch the relational schema or any story in §5 above.
...
3. **Poetry** replaces `requirements.txt` (`pyproject.toml`); `sentence-transformers`
...
### Tenant scoping — ChromaDB only

A `tenant_id` concept exists **only** inside `app/services/chromadb_service.py`
(config-driven via `CHROMADB_TENANT_ID`, default `"default"`): every vector
is stamped with it on `add_image`, and every `find_matches` query filters by
it alongside the existing per-product scoping. This is a namespace for
separating environments/deployments that share one ChromaDB instance — it is
**not** a relational multi-tenancy model. There is no `tenants` table, no
`tenant_id` on any other table, no tenant-scoped auth/login, and no per-tenant
credential overrides; `users`/`products`/everything else remain exactly as
specified in `Database_Schema.md`.
